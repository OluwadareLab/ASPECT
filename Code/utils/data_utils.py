...
def divide_dataset_into_folds(test_dataset, num_folds):
    """
    Divide the test dataset into 'num_folds' equal parts while keeping the label distribution stratified.
    
    Parameters:
        test_dataset (pd.DataFrame): The test dataset containing features and labels.
        num_folds (int): The number of folds to create (default is 5).
        label_column (str): The name of the column in 'test_dataset' that contains the labels (default is 'label').
    
    Returns:
        list of tuples (pd.DataFrame, pd.DataFrame): A list of 'num_folds' tuples, each representing a train-test split.
    """
    skf = StratifiedKFold(n_splits=num_folds, shuffle=True)
    fold_splits = []

    X = test_dataset.drop(columns=["CLASS"])
    y = test_dataset["CLASS"]

    for train_index, test_index in skf.split(X, y):
        train_fold = test_dataset.iloc[train_index]
        test_fold = test_dataset.iloc[test_index]
